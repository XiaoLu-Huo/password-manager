...
- [ ] 2. 实现加密引擎与安全工具
  ...
  - [ ] 2.5 实现密码强度评估器 (`PasswordStrengthEvaluator`)
...
- [ ] 3. 检查点 - 确保加密引擎和工具类测试通过
  - 确保所有测试通过，如有问题请向用户确认。

- [ ] 4. 实现用户认证与主密码管理
  - [ ] 4.1 实现 `AuthService` 及 `AuthServiceImpl`
...
- [ ] 7. 实现凭证 CRUD 与搜索
  - [ ] 7.1 实现 `CredentialService` 及 `CredentialServiceImpl`
